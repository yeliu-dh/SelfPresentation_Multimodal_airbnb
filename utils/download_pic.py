...
# 单张图片下载函数
def download_image(id, url, pic_folder='images_raw', timeout=10):
    ...
    out_path = os.path.join(pic_folder, filename)
    ...
    try:
        ...
        print(f"[ERROR] failed download: {url}, 错误: {e}")
        return None
...
# 批量并行下载函数
def download_images_batch(path_df="data_processed\paris_2406\listings_processed_paris_2406_62738.csv",pic_folder='images/paris_2306', max_workers=10):
    """
    download host profile pic 
    """
    ...
    if len(df_pic)>0:
        ...
        # ---save---
        # oi适合多线程
        failed=list(df[df['host_has_profile_pic']=='f']['host_id'])
        if "host_pic_is_valid" in df.columns:
            failed.extend(list(df[df['host_pic_is_valid'].isna()]['host_id']))
    
        print(f"[info] no / no valid host pic: {len(failed)}/{len(df_pic)}.\n")
        
        
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(download_image, id, url, pic_folder): id for id, url in id_url_list}#***
        ...
        # true: downloaded, false: failed, else: not download yet
        ...
        print(f'[info] ALL pics downloaded:)\n')
    return df
